...
import com.plenigo.pdflayout.spec.ELineCapStyle;
import com.plenigo.pdflayout.spec.ELineJoinStyle;
...
public class PDPageContentStreamExt implements Closeable {
     ...
     * Create a new PDPage content stream.
     *
     * @param aDoc          The document the page is part of.
     * @param sourcePage    The page to write the contents to.
     * @param appendContent Indicates whether content will be overwritten. If false all previous content is deleted.
     * @param compress      Tell if the content stream should compress the page contents.
     ...
     * Create a new appearance stream. Note that this is not actually a "page" content stream.
     *
     * @param aDoc         The document the appearance is part of.
     ...
     * @throws IllegalStateException If the method was called within a text block.
     */
    public void drawForm(final PDFormXObject form, final float x, final float y, final float width, final float height) throws IOException {
        if (m_bInTextMode)
            throw new IllegalStateException("Error: drawForm is not allowed within a text block.");

        saveGraphicsState();

        double drawingScaleFactorMin = Math.min(height, width);
        double drawingScaleFactor = 1;
        if (width >= height) {
            drawingScaleFactor = width / height;
            double formScaleFactor = form.getBBox().getWidth() / form.getBBox().getHeight();
            drawingScaleFactor = Math.min(drawingScaleFactor, formScaleFactor);
        }

        // We need the max scale factor so the svg fits into the given size.
        // This is not a perfect solution but good enough.
        double formScaleFactor = Math.max(form.getBBox().getHeight(), form.getBBox().getWidth());
        double formScaleFactorWidth = drawingScaleFactorMin / form.getBBox().getWidth() / formScaleFactor;
        double formScaleFactorHeight = drawingScaleFactorMin / form.getBBox().getHeight() / formScaleFactor;

        AffineTransform transform = new AffineTransform(form.getBBox().getWidth(), 0, 0, form.getBBox().getHeight(), x, y);
        transform.concatenate(AffineTransform.getScaleInstance(formScaleFactorWidth * drawingScaleFactor,
                formScaleFactorHeight * drawingScaleFactor));
        transform(new Matrix(transform));
        writeOperand(m_aResources.add(form));
        writeOperator((byte) 'D', (byte) 'o');
     ...
     *
     * @param eLineJoinStyle 0 for miter join, 1 for round join, and 2 for bevel join.
     *
     * @throws IOException              If the content stream could not be written.
     * @throws IllegalStateException    If the method was called within a text block.
     ...
     *
     * @param eLineCapStyle Line cap style to use
     *
     * @throws IOException              If the content stream could not be written.
     * @throws IllegalStateException    If the method was called within a text block.
     ...
     *
     * @param real the value to be written
     *
     ...
     *
     * @param text the value to be written
     *
     * @throws IOException In case of IO error
     */
